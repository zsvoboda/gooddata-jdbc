...
public class TestSQLParser {
    ...
    public void testParse() throws JSQLParserException {
        SQLParser.ParsedSQL parsedSQL = SQLParser.parseQuery("SELECT c1,c2 FROM t1");
        assert(parsedSQL.getColumns().contains("c1"));
        assert(parsedSQL.getColumns().contains("c2"));
        assert(parsedSQL.getTables().contains("t1"));

        parsedSQL = SQLParser.parseQuery("SELECT \"c1\",\"c2\" FROM \"t1\"");
        assert(parsedSQL.getColumns().contains("c1"));
        assert(parsedSQL.getColumns().contains("c2"));
        assert(parsedSQL.getTables().contains("t1"));

        parsedSQL = SQLParser.parseQuery("SELECT c1,c2 FROM t1 WHERE c1 IN ('v1','v2','v3')");
        ...
        parsedSQL = SQLParser.parseQuery("SELECT \"Product Category\",\"# of Orders\" " +
        ...
        parsedSQL = SQLParser.parseQuery("SELECT \"[/gdc/a]\", \"[/gdc/b]\" WHERE \"[/gdc/c]\" BETWEEN 1 AND 2");
        ...
        parsedSQL = SQLParser.parseQuery("SELECT c1,c2,m1 FROM t1 WHERE m1 = (3*5+3)/6");
        assert(parsedSQL.getColumns().contains("c1"));
        assert(parsedSQL.getColumns().contains("c2"));
        ...
        parsedSQL = SQLParser.parseQuery("SELECT c1,c2,m1 FROM t1 WHERE m1 BETWEEN (3*5+3)/6 AND (6*5+6)/6");
        assert(parsedSQL.getFilters().get(0).getValues().get(0).equals("3"));
        assert(parsedSQL.getFilters().get(0).getValues().get(1).equals("6"));

        parsedSQL = SQLParser.parseQuery("SELECT c1,c2,m1 FROM t1 WHERE c1 = 'Home'");
    ...
    public void testLimit() throws JSQLParserException {
        SQLParser.ParsedSQL parsedSQL = SQLParser.parseQuery("SELECT c1,c2,m1 FROM t1 WHERE c1='Home' LIMIT 10 OFFSET 3");
    ...
    public void testOrderBy() throws JSQLParserException {
        SQLParser.ParsedSQL parsedSQL = SQLParser.parseQuery("SELECT c1,c2,m1 FROM t1 WHERE c1='Home' ORDER BY 1, 2 ASC, 3 DESC");
        ...
        parsedSQL = SQLParser.parseQuery("SELECT c1,c2,m1 FROM t1 WHERE c1='Home' ORDER BY \"c1\" ASC, c2 DESC, m1");
        assert(parsedSQL.getOrderBys().get(0).getOrder().equals("ASC"));
    ...
    public void testParseException() throws JSQLParserException {
        SQLParser.parseQuery("SELECT c1,c2,m1 FROM t1 WHERE c1='Home ");
        SQLParser.parseQuery("SELECT c1,c2,m1 FROM t1 WHERE c1 IN () ");
        SQLParser.parseQuery("SELECT c1,c2,m1 FROM t1 WHERE c1 BETWEEN 2 ");
    }

    @Test
    public void testSparkSubquery() throws JSQLParserException {
        SQLParser.ParsedSQL parsedSQL = SQLParser.parseQuery("SELECT * FROM (SELECT \"PRODUCT_CATEGORY\", \"REVENUE\") " +
    ...
    public void testPreparedSQL() throws JSQLParserException {
        SQLParser.ParsedSQL parsedSQL = SQLParser.parseQuery("SELECT \"PRODUCT_CATEGORY\", \"REVENUE\"  WHERE " +
                "\"PRODUCT_CATEGORY\" IN (?)");
        parsedSQL = SQLParser.parseQuery("SELECT \"PRODUCT_CATEGORY\", \"REVENUE\"  WHERE " +
